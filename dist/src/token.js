 ...
 */
export function resolveA365Credentials(cfg) {
 ...
 */
export function resolveGraphTokenConfig(cfg) {
    const tenantId = cfg?.tenantId?.trim() || process.env.A365_TENANT_ID?.trim();
    const blueprintClientAppId = cfg?.graph?.blueprintClientAppId?.trim() ||
        cfg?.appId?.trim() ||
        process.env.BLUEPRINT_CLIENT_APP_ID?.trim() ||
        process.env.A365_APP_ID?.trim();
    const blueprintClientSecret = cfg?.graph?.blueprintClientSecret?.trim() ||
        cfg?.appPassword?.trim() ||
        process.env.BLUEPRINT_CLIENT_SECRET?.trim() ||
        process.env.A365_APP_PASSWORD?.trim();
    const aaInstanceId = cfg?.graph?.aaInstanceId?.trim() || process.env.AA_INSTANCE_ID?.trim();
 ...
 */
export function resolveTokenCallbackConfig(cfg) {
    const callbackUrl = cfg?.tokenCallback?.url?.trim() || process.env.TOKEN_CALLBACK_URL?.trim();
    if (!callbackUrl) {
        return undefined;
    }
    return {
 ...
 */
export async function getGraphToken(cfg, username, scope) {
    const log = getLogger();
 ...
 */
async function fetchTokenFromCallback(config, username, scope) {
    const headers = {
 ...
 */
async function fetchGraphTokenT1T2(config, username, scope) {
    const log = getLogger();
 ...
 */
export function invalidateGraphTokenCache(username, scope) {
    if (username && scope) {
