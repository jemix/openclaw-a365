 ...
 */
export function buildConversationReference(activity) {
    return {
        conversationId: activity.conversation?.id || "",
        serviceUrl: activity.serviceUrl || "",
 ...
 */
function setEnvForAccount(creds, port) {
 ...
 */
function registerMessageHandler(agentApp, ActivityTypes, TurnContext, opts) {
    ...
    // Handle all messages
    agentApp.onActivity(ActivityTypes.Message, async (context, _state) => {
        ...
        if (allowFrom && allowFrom.length > 0 && !allowFrom.includes("*")) {
            const userAllowed = allowFrom.includes(metadata.userId) ||
 ...
 */
export async function monitorA365Provider(opts) {
    const core = getA365Runtime();
    ...
    const { AgentApplication, MemoryStorage, TurnContext, TurnState, CloudAdapter } = await import("@microsoft/agents-hosting");
    ...
    if (hasMultiAccounts) {
        ...
        app.post("/api/messages", (req, res) => {
            ...
            if (entry) {
                ...
                if (match) {
                    entry.adapter.process(proxyReq, res, async (context) => match.agentApp.run(context));
                    return;
                }
            }
            ...
            if (fallbackEntry) {
                fallbackEntry.adapter.process(proxyReq, res, async (context) => fallback.agentApp.run(context));
            }
            else {
    ...
    const adapter = (agentApp.adapter ?? new CloudAdapter());
    setAdapter(adapter);
    const blueprintClientId = a365Cfg?.graph?.blueprintClientAppId?.trim() ||
        process.env.BLUEPRINT_CLIENT_APP_ID?.trim() ||
        creds.appId;
